...
let calculate_opam ~build_only ~local_opam_files ~local_packages ~exclude ~ocaml_version =
  OpamGlobalState.with_ `Lock_none (fun global_state ->
      OpamSwitchState.with_ `Lock_none global_state (fun switch_state ->
          check_repo_config ~global_state ~switch_state;
          Opam_solve.calculate ~build_only ~local_opam_files ~local_packages ~exclude ?ocaml_version
            switch_state))
